      ...
      .map((coin) => {
        return (
          ...
          >
            ...
            <div className="volume">{millify(coin["24hVolume"])}</div>
            <div className="mktcap">{millify(coin.marketCap)}</div>
          </div>
        );
      });
  const pageCount = Math.ceil(
    crypto.coins.filter((coin) =>
      coin.name.toLowerCase().includes(search.toLowerCase())
    ).length / coinsPerPage
  );
    ...
    <TableStyles colors={colors(theme)}>
      <div className="header">
        ...
        <span className="moreInfo">
          Click a Crypto coin for more information
        </span>
        <input
          type="text"
