...
const MyWatchList = ({voicePageNavigation,setVoicePageNavigation}) => {
  ...
  const{apiData} = useFetchAuthorization('https://crypto-mania-api-production.up.railway.app/api/account/cryptos',setOnload,onDelete,setOnDelete,setError,error)

  const deleteItem = (id)=>{
    DeleteApiCall(`https://crypto-mania-api-production.up.railway.app/api/account/crypto/${id}`,value,setOnDelete,setError)
  }
